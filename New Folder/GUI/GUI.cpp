...
void GUI::PrintMessage(string msg) const	//Prints a message on status bar
{
	...
	pWind->DrawString(10, 510, msg); // You may need to change these coordinates later 
...
void GUI::UpdateStatusBar(int CurrentTimeStep,double h,CASTLE_STATUS s,int AF,int Afr,int AH,int TA,int FrF,int FrFre,int FrH,int TFr,int KF,int KFr,int KH,int TK)	
{
		...
		pWind->DrawString(10,530,"Castle Health= "+to_string(h));

		if((int)s==0)
		{
			pWind->DrawString(200,530,"Castle Status= Active");
		}
		else
		   pWind->DrawString(250,530,"Castle Status= Freezed");

		pWind->DrawString(10, 550, "Active Fighters= "+to_string(AF));

		pWind->DrawString(330, 550,"Active Freezers= "+to_string(Afr));

		pWind->DrawString(650, 550, "Active Healers= "+to_string(AH));

		pWind->DrawString(970, 550,"Total Active= "+to_string(TA));

		pWind->DrawString(10, 570,"Frosted Fighters= "+to_string(FrF));

		pWind->DrawString(330, 570,"Frosted Freezers= "+to_string(FrFre));

		pWind->DrawString(650, 570,"Frosted Healers= "+to_string(FrH));

		pWind->DrawString(970, 570,"Total Frosted= "+to_string(TFr));

		pWind->DrawString(10, 590,"Killed Fighters= "+to_string(KF));

		pWind->DrawString(330, 590, "Killed Freezers= "+to_string(KFr));

		pWind->DrawString(650, 590,"Killed Healers= "+to_string(KH));

		pWind->DrawString(970, 590,"Total Killed= "+to_string(TK));
}

void GUI::UpdateInterface(int CurrentTimeStep,double h,CASTLE_STATUS s,int AF,int Afr,int AH,int TA,int FrF,int FrFre,int FrH,int TFr,int KF,int KFr,int KH,int TK) 
{
	ClearDrawingArea();
	UpdateStatusBar(CurrentTimeStep,h,s, AF, Afr, AH, TA, FrF, FrFre, FrH, TFr, KF, KFr, KH, TK);
	DrawCastleArea();
	DrawAllItems();
}
...
void GUI::AddToDrawingList( const Enemy* pE)
{
	...
	if(const Healer* arrH = dynamic_cast <const Healer*> (pE))
		     color=1;
	else if (const Freezer* arrH = dynamic_cast <const Freezer*> (pE))
		     color =2;
	else if (const Fighter* arrH = dynamic_cast <const Fighter*> (pE))
		     color =0;


	/////////////
	/////////////
	pDitem->clr = DrawingColors[color];
